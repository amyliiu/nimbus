...
# Parse optional command line argument for kernel base
if [ $# -ge 1 ]; then
    KERNEL_BASE="$1"
else
    echo "Usage: $0 <kernel_base_directory>"
    exit 1
fi
...
KERNEL="${KERNEL_BASE}/$(ls "${KERNEL_BASE}"/vmlinux* | tail -1 | xargs -n1 basename)"
...
ROOTFS="${KERNEL_BASE}/$(ls "${KERNEL_BASE}"/*.ext4 | tail -1 | xargs -n1 basename)"
